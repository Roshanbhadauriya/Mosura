...
const content = FEATURES_CONTENT["api-management-suite"];
...
export default function APIManagementSuitePage() {
  return (
          ...
          >
            {content.overview.capabilities.map((cap, idx) => (
              <motion.div
                ...
                className="group flex items-start gap-4 p-6 rounded-2xl border border-white/[0.06] bg-white/[0.02] hover:border-white/10 hover:bg-white/[0.04] transition-all duration-300"
              >
      ...
      <section className="w-full px-6 py-24 border-t border-white/5">
        <div className="max-w-6xl mx-auto">
          <motion.div
            ...
            className="text-center mb-16"
          >
            <p className="text-sm font-semiboldtracking-[0.2em] text-[#ed7f18] mb-3">
              Capabilities
            </p>
            ...
            {content.keyFeatures.map((feature) => {
              ...
              return (
                <motion.div
                  key={feature.title}
                  variants={itemVariants}
                  className="group relative rounded-2xl border border-white/[0.06] bg-white/[0.02] p-8 hover:border-white/10 hover:bg-white/[0.04] transition-all duration-300"
                >
      ...
      <section className="w-full px-6 py-24 border-t border-white/5">
        <div className="max-w-5xl mx-auto">
          ...
          >
            ...
            <span className="px-4 py-2 rounded-lg border border-white/10 bg-white/[0.03] text-slate-300">
          ...
          >
            {content.howItWorks.steps.map((step) => (
              ...
              >
                <span className="text-5xl font-black text-white/[0.04] absolute top-6 right-6 group-hover:text-[#ed7f18]/10 transition-colors duration-300">
                  {step.step}
                </span>
          ...
          >
            {content.whyChoose.items.map((item) => (
              <motion.div
                key={item.label}
                variants={itemVariants}
                className="group flex items-start gap-4 p-6 rounded-2xl border border-white/[0.06] bg-white/[0.02] hover:border-[#ed7f18]/20 hover:bg-white/[0.04] transition-all duration-300"
              >
                <div className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#ed7f18]/10 text-[#ed7f18] group-hover:bg-[#ed7f18]/20 transition-colors">
                  <Check className="h-4 w-4" />
                </div>
                <div>
          ...
          >
            {content.industries.items.map((industry) => (
              <motion.div
                key={industry.name}
                variants={itemVariants}
                className="group text-center p-6 rounded-2xl border border-white/[0.06] bg-white/[0.02] hover:border-[#ed7f18]/20 hover:bg-white/[0.04] transition-all duration-300"
              >
                <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-xl bg-[#ed7f18]/10 text-[#ed7f18] group-hover:scale-110 transition-transform">
                  <Building2 className="h-5 w-5" />
                </div>
                <h4 className="font-bold text-white mb-2 group-hover:text-[#ed7f18] transition-colors">
                  {industry.name}
                </h4>
      ...
      <section className="w-full px-6 py-24">
        <div className="max-w-4xl mx-auto text-center">
          <motion.div
            ...
            viewport={{ once: true }}
            transition={{ duration: 0.6 }}
            className="relative rounded-3xl border border-white/[0.06] bg-gradient-to-br from-white/[0.03] via-transparent to-[#ed7f18]/[0.03] p-12 md:p-20 overflow-hidden"
          >
            {/* Background glow */}
