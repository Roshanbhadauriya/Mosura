...
const content = FEATURES_CONTENT["micro-gateway-iot"];
...
export default function MicroGatewayIoTPage() {
  return (
      ...
      <section className="w-full px-6 py-24">
        <div className="max-w-5xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-center mb-16">
            ...
            <h2 className="text-3xl md:text-5xl font-bold text-white tracking-tight mb-6">Intelligence at the Edge</h2>
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.2 }} className="mb-16">
            <div className="flex flex-col items-center gap-4">
              <div className="w-full max-w-md px-6 py-5 rounded-2xl border border-[#ed7f18]/30 bg-[#ed7f18]/5 text-center">
                ...
                <p className="text-xs text-slate-500 mt-1">Policy & Security · Device Registry · Analytics & Telemetry</p>
              </div>
              <ArrowDown className="h-5 w-5 text-slate-600" />
              <div className="grid grid-cols-3 gap-4 w-full max-w-lg">
                {["Device Cluster", "Region Edge", "Partner Edge"].map((label) => (
                  <div key={label} className="px-4 py-4 rounded-xl border border-white/10 bg-white/3 text-center">
                    <p className="text-white text-xs font-bold">Micro Gateway</p>
                    <p className="text-[10px] text-slate-500 mt-1">({label})</p>
                  </div>
                ))}
              </div>
              <ArrowDown className="h-5 w-5 text-slate-600" />
              <p className="text-xs text-slate-500 italic">API / Events / Telemetry → Routed, Secured, Transformed</p>
            </div>
          </motion.div>
