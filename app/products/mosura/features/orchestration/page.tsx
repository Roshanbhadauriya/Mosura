...
const content = FEATURES_CONTENT["orchestration"];
    ...
    <div className="relative flex flex-col items-center bg-black">
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.2 }} className="mb-16">
            <div className="flex flex-col items-center gap-4">
              <div className="w-full max-w-md px-6 py-5 rounded-2xl border border-[#ed7f18]/30 bg-[#ed7f18]/5 text-center">
                ...
                <p className="text-xs text-slate-500 mt-1">Workflows · Policy-as-Code · AI & Event Routing · Analytics</p>
              </div>
              <ArrowDown className="h-5 w-5 text-slate-600" />
              <div className="grid grid-cols-3 gap-4 w-full max-w-lg">
                {["API Gateway", "AI Gateway", "Event Gateway"].map((label) => (
                  <div key={label} className="px-4 py-4 rounded-xl border border-white/10 bg-white/3 text-center">
                    <p className="text-white text-xs font-bold">{label}</p>
                    <p className="text-[10px] text-slate-500 mt-1">Data Plane</p>
                  </div>
                ))}
              </div>
              <ArrowDown className="h-5 w-5 text-slate-600" />
              <p className="text-xs text-slate-500 italic">Requests / Events → Routed, Transformed, Validated, & Coordinated</p>
            </div>
          </motion.div>
      ...
      <section className="w-full px-6 py-24 border-t border-white/5">
        <div className="max-w-5xl mx-auto">
          ...
          <motion.div variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true }} className="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
